...
for bs in range(len(batsize)):
	for ob in range(len(overbuild)):
		...
		daily_unmet = reliable_df_gb.values / demand_df_gb.values
		...
		pcm = plt.pcolormesh(x, y, data2plot.T, cmap = 'gnuplot2', vmin = 0, vmax = 0.7)# rasterized = True)
		plt.xlim(1,366)
		...
		plt.savefig('{}/Daily_demandnotmet_overbuild-{}_storagesize-{}hours_SF-{}_{}_gnuplot2-colormap.svg'
