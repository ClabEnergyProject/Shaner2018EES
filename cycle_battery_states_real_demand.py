...
def state_cycle(state):
	...
	if (state != 'Hawaii') and (state != 'Alaska'): # aren't doing Alaska or Hawaii (CONUS only)

		filename_start = '{}_area-weighted-mean_real-demand'.format(state) # start to the file names
		
		# # if path exists, don't run again
		# if os.path.exists('{}/{}/{}_battery_size.npy'.format(path, country_folder, 
		# 	filename_start)):

		# 	pass

		# # if path doesn't exist, run
		# else: 

		# import solar and wind data
		CF_solar = np.load('{}/{}/{}_CFsolar_area-weighted-mean.npy'.format(path, 
			country_folder, state))
...
if __name__ == '__main__':
	...
	# import state regions shape file, extract state names 
	shapefile = gpd.GeoDataFrame.from_file('{}/country_mask_files/USA_states/states.shp'
		.format(sfpath))
	state_names = shapefile['STATE_NAME']

	# multiprocess
	p = Pool(10)
	p.map(state_cycle, state_names)
